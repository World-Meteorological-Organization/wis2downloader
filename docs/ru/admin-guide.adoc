...
Скрипт создаёт файл `.env` со случайно сгенерированными секретами. Проверьте и при необходимости скорректируйте настройки перед запуском.

Установите плагин Docker для Loki (требуется для отправки журналов):
