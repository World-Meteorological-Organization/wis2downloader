...
`selected_topics` holds the MQTT topics currently selected in the UI and drives the right sidebar.
...
1. Create `modules/ui/views/myview.py` with a `render(container)` function
2. Add a navigation entry in `components/navigation_drawer.py`
